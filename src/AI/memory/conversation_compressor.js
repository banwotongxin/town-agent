...
exports.ConversationCompressor = void 0;
...
class ConversationCompressor {
     ...
     * @returns 压缩后的对话摘要
     */
    compress(messages, existingSummary, maxTokens = 500) {
        // 存储格式化后的对话文本
    ...
    calculateTokenCount(text) {
        return token_utils_1.TokenUtils.calculateTokenCount(text);
    }
    /**
     * 第一层压缩：删除过期的工具记忆
     * 保留最近的N个工具调用和结果
     ...
     * @returns 过滤后的消息数组
     */
    removeExpiredToolMemories(messages, maxToolCalls = 3) {
     ...
     * @returns 关键对话消息数组
     */
    extractKeyConversations(messages) {
        // 策略1：保留所有用户和助手消息
        const keyMessages = messages.filter(msg => msg.type === 'human' || msg.type === 'ai');
     ...
     * @returns 压缩后的文本摘要
     */
    async compressWithLLM(messages, existingSummary) {
        if (!this.llmModel) {
     ...
     * 构建摘要提示词
     */
    buildSummaryPrompt(messages, existingSummary) {
     ...
     * @returns 压缩后的消息数组或摘要
     */
    async compressThreeLayers(messages, useLLM = true) {
        ...
        if (useLLM && compressed.length > 5) {
            try {
                ...
                // 将摘要转换为单条消息
                return [new base_agent_1.AIMessage(`[对话摘要]\n${summary}`, {
...
exports.ConversationCompressor = ConversationCompressor;
